...
const BonusParticles = function(){
    ...
    for (let i=0; i<10; i++){
        ...
        let partGreen = new THREE.Mesh(smallParticleGeom, greenMat);
...
BonusParticles.prototype.explose = function(){
    ...
    for(let i=0; i<this.parts.length; i++){
        ...
        p.scale.set(1,1,1);
        p.visible = true;
        let s = explosionSpeed + Math.random()*.5;
...
const Monster = function(){
    ...
    for(let i=0; i<3; i++){
        let toothf = new THREE.Mesh(toothGeom, whiteMat);
        toothf.position.x = -2.8 + i*2.5;
        toothf.position.y = 1;
        toothf.position.z = 19;

        let toothl = new THREE.Mesh(toothGeom, whiteMat);
...
Monster.prototype.nod = function(){
    ...
    // HEAD
    let tHeadRotY = -Math.PI/3 + Math.random()*.5;
    let tHeadRotX = Math.PI/3 - .2 +  Math.random()*.4;
...
const Carrot = function() {
    ...
    let bodyGeom = new THREE.CylinderGeometry(5,3, 10, 4,1);
...
const Hedgehog = function() {
    ...
    for (let i=0; i<9; i++){
        ...
        let sr = new THREE.Mesh(spikeGeom, blackMat);
        sr.position.x = 3;
        ...
        let sl = new THREE.Mesh(spikeGeom, blackMat);
        sl.position.x = -3;
...
function createFirs(){
    ...
    for(let i=0; i< nTrees; i++){
        ...
        let vec = fir.mesh.position.clone();
...
function checkCollision(){
    let db = hero.mesh.position.clone().sub(carrot.mesh.position.clone());
    let dm = hero.mesh.position.clone().sub(obstacle.mesh.position.clone());
...
const Trunc = function(){
    let truncHeight = 50 + Math.random()*150;
    let topRadius = 1+Math.random()*5;
    let bottomRadius = 5+Math.random()*5;
    let mats = [blackMat, brownMat, pinkMat, whiteMat, greenMat, lightBrownMat, pinkMat];
    let matTrunc = blackMat;//mats[Math.floor(Math.random()*mats.length)];
    let nhSegments = 3;//Math.ceil(2 + Math.random()*6);
    let nvSegments = 3;//Math.ceil(2 + Math.random()*6);
    let geom = new THREE.CylinderGeometry(topRadius,bottomRadius,truncHeight, nhSegments, nvSegments);
